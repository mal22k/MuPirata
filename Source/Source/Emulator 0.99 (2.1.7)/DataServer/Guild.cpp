...
void CGuild::GDGuildMemberDelRecv(SDHP_GUILD_MEMBER_DEL_RECV* lpMsg,int aIndex) // OK
{
	...
	if((pMsg.result=gGuildManager.DelGuildMember(lpMsg->Name,lpMsg->Member)) != 1)
	{
		gSocketManager.DataSend(aIndex,(BYTE*)&pMsg,sizeof(pMsg));
		return;
	}
...
{
	GUILD_INFO* lpGuild = gGuildManager.GetGuildMember(member);

	if(lpGuild != 0)
	{
		GUILD_MEMBER_INFO* lpMember = gGuildManager.GetMemberInfo(lpGuild->Name,member);
		...
		this->MemberUpdate(lpGuild->Name,member,server);
	}
}
...
void CGuild::MemberDisconnect(char* member) // OK
{
	...
	if(lpGuild != 0)
	{
		...
		this->MemberUpdate(lpGuild->Name,member,-1);
	}
}

void CGuild::MemberUpdate(char* name,char* member,int server) // OK
{
