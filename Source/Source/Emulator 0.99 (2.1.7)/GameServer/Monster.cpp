...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if((DropIndex=gMonsterManager.GetMonsterItem(DropLevel,ExcItemDrop)) >= GET_ITEM(0,0) && (GetLargeRand()%((lpObj->ItemRate==0)?1:lpObj->ItemRate)) < ItemDropRate)
	{
		...
		GDCreateItemSend(lpTarget->Index,lpObj->Map,(BYTE)lpObj->X,(BYTE)lpObj->Y,ItemIndex,ItemLevel,0,ItemOption1,ItemOption2,ItemOption3,lpTarget->Index,ItemNewOption,ItemSetOption);
	}
...
bool gObjSetMonster(int aIndex,int MonsterClass) // OK
{
	...
	if(lpObj->AttackType != 0)
	{
		...
		if(MonsterClass == 145 || MonsterClass == 175 || MonsterClass == 183 || MonsterClass == 191 || MonsterClass == 261 || MonsterClass == 269) // Kalima Death Centurion
		{
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
		}

		if(MonsterClass == 146 || MonsterClass == 176 || MonsterClass == 184 || MonsterClass == 192 || MonsterClass == 262 || MonsterClass == 270) // Kalima Bloody Soldier
		{
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
		}

		if(MonsterClass == 147 || MonsterClass == 177 || MonsterClass == 185 || MonsterClass == 193 || MonsterClass == 263 || MonsterClass == 271) // Kalima Aegis
		{
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
		}

		if(MonsterClass == 148 || MonsterClass == 178 || MonsterClass == 186 || MonsterClass == 194 || MonsterClass == 264 || MonsterClass == 272) // Kalima Lord Centurion
		{
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
		}

		if(MonsterClass == 149 || MonsterClass == 179 || MonsterClass == 187 || MonsterClass == 195 || MonsterClass == 265 || MonsterClass == 273) // Kalima Necron
		{
			gSkillManager.AddSkill(lpObj,SKILL_POISON);
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
		}

		if(MonsterClass == 160 || MonsterClass == 180 || MonsterClass == 188 || MonsterClass == 196 || MonsterClass == 266 || MonsterClass == 274) // Kalima Schriker
		{
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
		}

		if(MonsterClass == 161 || MonsterClass == 181 || MonsterClass == 189 || MonsterClass == 197 || MonsterClass == 267 || MonsterClass == 275) // Shadow of Kundun
		{
			gSkillManager.AddSkill(lpObj,SKILL_POISON);
			gSkillManager.AddSkill(lpObj,SKILL_ENERGY_BALL);
			gSkillManager.AddSkill(lpObj,SKILL_FIRE_SLASH);
			gSkillManager.AddSkill(lpObj,SKILL_MONSTER_SUMMON);
			gSkillManager.AddSkill(lpObj,SKILL_MAGIC_DAMAGE_IMMUNITY);
			gSkillManager.AddSkill(lpObj,SKILL_PHYSI_DAMAGE_IMMUNITY);
		}
...
void gObjMonsterMagicAttack(LPOBJ lpObj,int skill) // OK
{
	...
	pMsg.header.set(0x1D,sizeof(pMsg));
...
void gObjMonsterAttack(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	else if(lpObj->Class == 144 || lpObj->Class == 174 || lpObj->Class == 182 || lpObj->Class == 190 || lpObj->Class == 260 || lpObj->Class == 268)
	{
		...
		else
		{
			...
			pMsg.header.set(0xD9,sizeof(pMsg));
...
void gObjTrapFindTarget(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if((gObj[lpObj->VpPlayer2[n].index].Authority & 4) != 0)
		{
			continue;
		}
