...
bool CQuestObjective::MonsterItemDrop(LPOBJ lpMonster) // OK
{
	...
	for each(QUEST_OBJECTIVE_INFO lpInfo in this->m_QuestObjectiveInfo)
	{
		...
		{
			GDCreateItemSend(aIndex,lpMonster->Map,(BYTE)lpMonster->X,(BYTE)lpMonster->Y,lpInfo.Index,lpInfo.Level,lpInfo.Option1,lpInfo.Option2,lpInfo.Option3,0,aIndex,lpInfo.NewOption,0);
			return 1;
		}
	}
