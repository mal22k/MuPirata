...
void CChaosBox::ChaosItemMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		GDCreateItemSend(lpObj->Index,0xFF,0,0,item.m_Index,(BYTE)item.m_Level,(BYTE)item.m_Durability,item.m_Option1,item.m_Option2,item.m_Option3,-1,item.m_NewOption,item.m_SetOption);
...
void CChaosBox::DevilSquareMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		GDCreateItemSend(lpObj->Index,0xFF,0,0,GET_ITEM(14,19),EyeLevel,1,0,0,0,-1,0,0);
...
void CChaosBox::DinorantMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		GDCreateItemSend(lpObj->Index,0xFF,0,0,ItemIndex,0,255,0,0,0,-1,ItemNewOption,0);
...
void CChaosBox::FruitMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		GDCreateItemSend(lpObj->Index,0xFF,0,0,GET_ITEM(13,15),ItemLevel,0,0,0,0,-1,0,0);
...
void CChaosBox::BloodCastleMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) <= lpObj->ChaosSuccessRate)
	{
		GDCreateItemSend(lpObj->Index,0xFF,0,0,GET_ITEM(13,18),ScrollLevel,1,0,0,0,-1,0,0);
...
void CChaosBox::PetMix(LPOBJ lpObj,int type) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		GDCreateItemSend(lpObj->Index,0xFE,0,0,(GET_ITEM(13,4)+type),0,0,0,0,0,-1,0,0);
