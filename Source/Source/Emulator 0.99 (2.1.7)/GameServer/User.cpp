	...
	if(lpObj->PKLevel > 3)
	{
		...
		if((GetLargeRand()%100) < rate)
		{
			...
			else
			{
				...
				if(gMap[lpObj->Map].ItemDrop(lpItem->m_Index,lpItem->m_Level,lpItem->m_Durability,lpObj->X,lpObj->Y,lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_Serial,lpObj->Index,lpItem->m_PetItemLevel,lpItem->m_PetItemExp) != 0)
				{
					pMsg.result = 1;
...
void gObjSetPosition(int aIndex,int x,int y) // OK
{
	...
	pMove.header.set(0xD0,sizeof(pMove));
