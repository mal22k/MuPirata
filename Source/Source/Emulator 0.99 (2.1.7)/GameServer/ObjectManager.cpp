...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	for(int n=0;n < 7;n++)
	{
		if(TempInventory[n] != 0x1F && TempInventory[n] != 0xFF)
		{
	...
	if(TempInventory[INVENTORY_SLOT_WING] == 0x1F)
	{
		lpObj->CharSet[5] |= 12;
	}
	...
	if(TempInventory[INVENTORY_SLOT_HELPER] == 0x1F)
	{
		lpObj->CharSet[5] |= 3;
	}
