...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gGuild.gObjTargetGuildWarCheck(lpObj,lpTarget) == 0)
	{
		if(lpTarget->Guild != 0 && lpTarget->Guild->WarState != 0)
		{
			...
			if(gCustomArena.CheckMap(lpTarget->Map) == 0 && CC_MAP_RANGE(lpTarget->Map) == 0 && gDuel.CheckDuel(lpObj,lpTarget) == 0)
			{
				return 0;
			}
		}
...
bool CAttack::MissCheckPvP(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int send,int count,BYTE* miss) // OK
{
	...
	int AttackSuccessRate = (int)(((100*(((lpObj->AttackSuccessRatePvP*10000.0f)/(lpObj->AttackSuccessRatePvP+lpTarget->DefenseSuccessRatePvP))/10000.0f))*2)*(((lpObj->Level*10000.0f)/(lpObj->Level+lpTarget->Level))/10000.0f));
...
int CAttack::GetTargetDefense(LPOBJ lpObj,LPOBJ lpTarget,WORD* effect) // OK
{
	...
	if((GetLargeRand()%100) < (lpObj->IgnoreDefenseRate+lpObj->EffectOption.AddIgnoreDefenseRate))
	{
		(*effect) = 1;
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	else
	{
		...
		if((GetLargeRand()%100) < (lpObj->CriticalDamageRate+lpObj->EffectOption.AddCriticalDamageRate))
		{
			(*effect) = 3;
		...
		if((GetLargeRand()%100) < (lpObj->ExcellentDamageRate+lpObj->EffectOption.AddExcellentDamageRate))
		{
			(*effect) = 2;
