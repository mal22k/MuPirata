...
void CSkillManager::UseAttackSkill(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpSkill->m_index == SKILL_TWISTING_SLASH && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() == 0)
	{
		return;
	}
	...
	{
		if(lpObj->ComboSkill.CheckCombo(lpObj->Class,lpSkill->m_index) != 0)
		{
...
void CSkillManager::CGMultiSkillAttackRecv(PMSG_MULTI_SKILL_ATTACK_RECV* lpMsg,int aIndex,bool type) // OK
{
	...
	if(lpSkill->m_index != SKILL_FLAME && lpSkill->m_index != SKILL_TWISTER && lpSkill->m_index != SKILL_EVIL_SPIRIT && lpSkill->m_index != SKILL_HELL_FIRE && lpSkill->m_index != SKILL_AQUA_BEAM && lpSkill->m_index != SKILL_BLAST && lpSkill->m_index != SKILL_INFERNO && lpSkill->m_index != SKILL_TRIPLE_SHOT && lpSkill->m_index != SKILL_IMPALE && lpSkill->m_index != SKILL_MONSTER_AREA_ATTACK && lpSkill->m_index != SKILL_PENETRATION && lpSkill->m_index != SKILL_FIRE_SLASH)
	{
		return;
	}
