...
void CCommandManager::CommandMake(LPOBJ lpObj,char* arg) // OK
{
	...
	if(count < 0)
	{
		...
		{
			GDCreateItemSend(lpObj->Index,0xEB,0,0,GET_ITEM(section,type),level,0,skill,luck,option,-1,exc,set);
		}

		gLog.Output(LOG_COMMAND,"[CommandMake][%s][%s] - Count: %d (ItemIndex: %02d,%03d,Level: %d,Skill: %d,Luck: %d,Option: %d,Exc: %d,Set: %d)",lpObj->Account,lpObj->Name,count,section,type,level,skill,luck,option,exc,set);
	}
	else
	{
		...
		GDCreateItemSend(lpObj->Index,0xEB,0,0,GET_ITEM(count,type),level,0,skill,luck,option,-1,exc,set);

		gLog.Output(LOG_COMMAND,"[CommandMake][%s][%s] - (ItemIndex: %02d,%03d,Level: %d,Skill: %d,Luck: %d,Option: %d,Exc: %d,Set: %d,JoH: %d,380: %d,Socket: %d)",lpObj->Account,lpObj->Name,count,type,level,skill,luck,option,exc,set);
	}
...
void CCommandManager::CommandDrop(LPOBJ lpObj,char* arg) // OK
{
	...
	if(count < 0)
	{
		...
		for(int n = 0; n < count; n++)
		{
			...
			GDCreateItemSend(lpObj->Index,lpObj->Map,px,py,GET_ITEM(section,type),level,0,skill,luck,option,lpObj->Index,exc,set);
		}

		gLog.Output(LOG_COMMAND,"[CommandDrop][%s][%s] - Count: %d (ItemIndex: %02d,%03d,Level: %d,Skill: %d,Luck: %d,Option: %d,Exc: %d,Set: %d)",lpObj->Account,lpObj->Name,count,section,type,level,skill,luck,option,exc,set);
	}
	else
	{
		int type = this->GetNumber(arg,1);
		...
		GDCreateItemSend(lpObj->Index,lpObj->Map,(BYTE)lpObj->X,(BYTE)lpObj->Y,GET_ITEM(count,type),level,0,skill,luck,option,lpObj->Index,exc,set);

		gLog.Output(LOG_COMMAND,"[CommandDrop][%s][%s] - (ItemIndex: %02d,%03d,Level: %d,Skill: %d,Luck: %d,Option: %d,Exc: %d,Set: %d)",lpObj->Account,lpObj->Name,count,type,level,skill,luck,option,exc,set);
	}
...
void CCommandManager::CommandMakeSet(LPOBJ lpObj,char* arg) // OK
{
	...
	for(int i=7;i<12; i++)
	{
		...
		{
			GDCreateItemSend(lpObj->Index,0xEB,0,0,GET_ITEM(i,type),level,0,0,luck,option,-1,exc,set);
		}
	}
	
	gLog.Output(LOG_COMMAND,"[CommandMakeSet][%s][%s] - (Type: %d,Level: %d,Luck: %d,Option: %d,Exc: %d,Set: %d)",lpObj->Account,lpObj->Name,type,level,luck,option,exc,set);
